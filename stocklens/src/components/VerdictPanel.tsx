  ...
  return (
    <div className="bg-navy-800 rounded-2xl p-4 shadow-card space-y-4 animate-slide-up">
        ...
        <div>
          ...
          <ul className="space-y-1.5">
            {scoring.verdictWhy.map((why, i) => (
              <li
                key={i}
                className="text-sm text-slate-300 leading-relaxed pl-3 border-l border-accent/30"
              >
                {why}
              </li>
            ))}
      ...
      <p className="text-xs text-slate-600 pt-2 border-t border-navy-700/50">
        This score measures risk/quality characteristics only. High confidence ≠ guaranteed profit.
